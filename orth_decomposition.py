...
def orth_decomp_of_unitary(X):
  ...
  Dr = np.diag(Ql.transpose() @ Xr @ Qr).copy()
  Di = np.diag(Ql.transpose() @ Xi @ Qr).copy()
...
def unit_kronecker_rank_approx(X): 
...
def drawHaarRandomUnitary(dim, real=False):
...
test_KAK_decomposition(drawHaarRandomUnitary(4))

# test on Marek's example
from scipy.linalg import expm

H = .1 * (PXX + .5 * (np.kron(PAULI_Z, np.eye(2)) + np.kron(np.eye(2), PAULI_Z)))
test_KAK_decomposition(expm(1j * H))
